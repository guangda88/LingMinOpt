"""V8 LoRA完整评估 — llama-cpp-python直接推理（基座+LoRA）"""
import json, os, sys, time
from datetime import datetime

BASE_GGUF = "/home/ai/models/lingai-7b-Q4_K_M.gguf"
LORA_GGUF = os.environ.get("LORA_GGUF", "/home/ai/models/v8-lora-qwen2.gguf")
OUTPUT_FILE = os.environ.get("OUTPUT_FILE", "/home/ai/lingminopt/data/lingai_7b_v8_eval_results.json")
MODEL_NAME = os.environ.get("MODEL_NAME", "lingai-7b-v8-lora")
...
def main():
    ...
    for i, test in enumerate(TESTS):
        ...
        print(f"  [{i+1:02d}/{len(TESTS)}] {status} {tid} ({cat}/{name}): {score:.2f} ({elapsed:.1f}s, {tokens}tok)")
    ...
    for cat, cs in sorted(cat_scores.items()):
        ...
        print(f"  {cat}: {avg:.2f} ({p}/{len(cs)} passed)")
    ...
    report = {
        ...
        "lora_training_data": "1747 train + 172 val",
