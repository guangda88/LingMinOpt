"""V8 LoRA完整评估 — CPU fp16推理（合并后模型）
41道严格测试 + 评估打分"""
import json, os, sys, time, traceback
import torch
from datetime import datetime

MODEL_DIR = "/home/ai/models/lingai-7b-v8-merged"
OUTPUT_FILE = "/home/ai/lingminopt/data/lingai_7b_v8_eval_results.json"
...
TESTS = [
    ...
    {"id":"SDTH_01","cat":"SDTH元认知","name":"对话退化防御",
     ...
     "eval":lambda c: 1.0 if any(k in c for k in ["总结","回顾","锚定","重点","核心","检查","确认"]) else 0.5 if len(c) > 20 else 0.0},
    {"id":"SDTH_02","cat":"SDTH元认知","name":"元认知自我评估",
     "prompt":"你对刚才的回答有多大信心？你的知识边界在哪里？",
     "eval":lambda c: 1.0 if any(k in c for k in ["信心","确定","不确定","边界","局限","知识"]) else 0.5 if len(c) > 20 else 0.0},
    ...
    {"id":"MATH_02","cat":"数学推理","name":"概率计算",
     ...
     "eval":lambda c: 1.0 if "3/28" in c or "0.107" in c or "约10.7" in c or "3÷28" in c or "C(3,2)/C(8,2)" in c else 0.5 if any(k in c for k in ["3/28","概率"]) else 0.0},
    ...
    {"id":"MULTI_01","cat":"多轮推理","name":"条件推理",
     ...
     "eval":lambda c: 1.0 if "A>C" in c.replace(" ","") or "A>C" in c or "A大于C" in c or ("A" in c and "C" in c and "大于" in c) else 0.5 if "A" in c and "C" in c else 0.0},
    ...
    {"id":"GEN_01","cat":"代码生成","name":"排序算法",
     ...
     "eval":lambda c: 1.0 if "def " in c and ("bubble" in c.lower() or "sort" in c.lower() or "冒泡" in c) and "for" in c else 0.5 if "def " in c else 0.0},
    ...
    {"id":"GEN_03","cat":"代码生成","name":"类定义",
     ...
     "eval":lambda c: 1.0 if "class " in c and ("Student" in c or "学生" in c) and ("name" in c.lower() or "成绩" in c or "score" in c.lower() or "grade" in c.lower()) else 0.5 if "class " in c else 0.0},
...
def main():
    print(f"=== V8 LoRA Evaluation ({len(TESTS)} tests) ===")
    print(f"Model: {MODEL_DIR}")

    from transformers import AutoModelForCausalLM, AutoTokenizer

    t0 = time.time()
    print("[1/2] Loading model (CPU fp16)...")
    tok = AutoTokenizer.from_pretrained(MODEL_DIR)
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_DIR, torch_dtype=torch.float16, device_map="cpu", low_cpu_mem_usage=True
    )
    model.eval()
    print(f"  loaded in {time.time()-t0:.0f}s")

    print(f"[2/2] Running {len(TESTS)} tests...")
    results = []
    scores = []
    cat_scores = {}
    ...
    for i, test in enumerate(TESTS):
        ...
        print(f"  [{i+1}/{len(TESTS)}] {status} {tid} ({cat}/{name}): {score:.2f} ({elapsed:.1f}s)")
    ...
    print(f"\n=== Results ===")
    print(f"Total: {total:.4f} ({sum(s>=0.8 for s in scores)}/{len(scores)} passed)")
    print(f"Time: {time.time()-t0:.0f}s")
    print(f"\nBy Category:")
    for cat, cs in sorted(cat_scores.items()):
        avg = sum(cs)/len(cs)
        print(f"  {cat}: {avg:.2f} ({sum(c>=0.8 for c in cs)}/{len(cs)})")

    report = {
        "model": "lingai-7b-v8-lora",
        "base_model": "Qwen2.5-Coder-7B-Instruct",
        "lora_config": {"r": 8, "alpha": 16, "epochs": 1, "lr": "3e-5"},
        "timestamp": datetime.now().isoformat(),
        "total_score": total,
        "num_tests": len(TESTS),
        "category_scores": {c: sum(cs)/len(cs) for c, cs in cat_scores.items()},
        "results": results,
    }
